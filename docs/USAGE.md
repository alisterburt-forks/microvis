...
### The `ModelFor` pattern

You'll note that most of the `core` objects are subclasses of `ModelFor`.
...
```python
class Canvas(ModelFor[CanvasBackend]):
  width: float = 500
  height: float = 500
  visible: bool = False
```
...
`ModelFor` then, is a base class (it's a `Generic` parametrized by
...
The logic for this is defined in `ModelFor._on_any_event` ...
