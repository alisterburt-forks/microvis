...
__all__ = ["Field", "ModelFor", "ModelBase", "SupportsVisibility"]
...
F = TypeVar("F", covariant=True, bound="ModelFor")
...
class ModelFor(ModelBase, Generic[T]):
...
# of signal names.
# XXX: also ... this might make more sense as a method on the ModelFor class
# where we have access to the bound "T" type variable (could remove some casts)
@lru_cache
def validate_backend_class(
    cls: type[ModelFor], backend_class: Any
) -> type[BackendAdaptor]:
